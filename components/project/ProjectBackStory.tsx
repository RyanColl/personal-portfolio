                            ...
                            <motion.div key={`bs-skill-${i+1}`} className='backstory-skill'>
                                
                                {
                                    <motion.a whileHover={{y: 2.5,scale: 1.15}} data-tooltip={skill} data-tooltip-location={width < 600 ? (skill==='Angular'?'top':'bottom') : (i===0?'top':'right')} target={'_blank'} href={`https://www.google.com/search?q=${skill}`} style={{marginTop: 4}}>
                                        ...
                                        <span id={skill==='Angular'?'angular-icon':skill==='Express'?'express-icon':undefined} className="iconify" color={skill==='React Native' ? '#3AF7F0' : skill==='TypeScript' ? '#007acc' : undefined} data-icon={skill==='React Native'? 'tabler:brand-react-native' : skill==='TypeScript' ? "simple-icons:typescript" : skill==='Angular' ? "logos:angular-icon" : skill==="Express" ? 'simple-icons:express' : skill==='NEXTJS' ? 'logos:nextjs-icon' : skill==='MongoDB' ? 'vscode-icons:file-type-mongo' : skill==='SCSS' ? 'vscode-icons:file-type-scss2'  : `logos:${skill.replaceAll(' ','-').toLowerCase()}`} data-width={(skill!=='Express' && skill!=='Angular') ? 40 : undefined} data-height={(skill!=='Express' && skill!=='Angular') ? 40 : undefined}></span> :
